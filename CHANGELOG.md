...
- ✨ **Auto-Resume**: Automatic detection when CAPTCHA is resolved (no manual Enter press required!)
- ⏱️ **Smart Polling**: Checks every 2 seconds for CAPTCHA resolution with progress updates
...
- 🛡️ **Timeout Protection**: 5-minute maximum wait for CAPTCHA with intelligent fallback
...
- 🐛 Fixed pagination stopping after CAPTCHA resolution due to poor error handling
- 🐛 Improved next page navigation with explicit exception handling and logging

### Changed
- 📝 Updated README with enhanced CAPTCHA handling and data directory explanations
...
- ⏲️ Increased wait times after CAPTCHA resolution and page navigation for stability

### Technical Details
- Added `_detect_captcha()` method with multiple detection strategies (iframes, selectors, keywords)
- Enhanced `_wait_for_captcha_resolution()` method with automatic polling and timeout handling
...
- Improved `_go_to_next_page()` with granular error handling and better logging
...
[0.1.2]: https://github.com/alexcolls/linkedin-spider/compare/v0.1.1...v0.1.2
